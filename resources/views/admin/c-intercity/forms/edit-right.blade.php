...
<div class="col-12 col-md-3">
    <div class="card mb-3">
        ...
        <div class="card-body p-2">
            <div class="w-100 d-flex align-items-center h-100 gap-2">
                ...
                <x-link :href="route('admin.cIntercity.index')" class="btn btn-outline w-50">
                    @lang('Quay lại')
                </x-link>
            </div>
        </div>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body p-2">
            <x-select name="trip_type" :required="true">
                @foreach ($trip_type as $key => $value)
                    <x-select-option :option="$order->trip_type->value" :value="$key" :title="$value" />
                @endforeach
            </x-select>
        </div>
    </div>
