<div class="col-12 col-md-3">
        ...
        <div class="card-body p-2">
            <x-select name="status" :required="true">
                @foreach ($status as $key => $value)
                    <x-select-option :value="$key" :title="$value" :selected="$category_system->status == $key" />
                @endforeach
            </x-select>
        </div>
    </div>
    <div class="card mb-3">
        ...
        <div class="card-body p-2">
            <x-input-image-ckfinder name="avatar" showImage="avatar" :value="$category_system->avatar" />
        </div>
    </div>
