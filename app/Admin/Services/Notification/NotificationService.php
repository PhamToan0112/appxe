...
class NotificationService implements NotificationServiceInterface
{
    ...
    public function store(Request $request)
    {
        ...
        if (empty($this->data['type'])) {
            $this->data['type'] = MessageType::UNCLASSIFIED->value;
        }
        switch ($this->data['types']) {
            case NotificationType::All->value:
                $recipients = [$this->driverRepository, $this->userRepository, $this->adminRepository];
                ...
                break;
            case NotificationType::Driver->value:
                $this->data['user_id'] = null;
    ...
    private function handleNotificationOption(string $objectId)
    {
        if ($this->data['option'] == \App\Enums\Notification\NotificationOption::All->value) {
            ...
            if (isset($this->data[$objectId])) {
                ...
                switch ($this->data['types']) {
                    case NotificationType::Driver->value:
                        $users = $this->driverRepository->findMany($objectIds);
                        break;
                    case NotificationType::Customer->value:
    ...
    public function update(Request $request): object|bool
    {
        ...
        if ($request->is_verified == VerifiedStatus::Active->value) {
            $userId = $notification->user_id;
            $driverId = $notification->driver_id;

            $title = config('notifications.successful_confirmation.title');
            $body = config('notifications.successful_confirmation.message');
            ...
            if ($userId) {
                ...
                $this->sendFirebaseNotificationToDriver($driver, $title, $body, MessageType::UNCLASSIFIED);
            } else {
                return back()->with('error', __('No recipient found.'));
            }
        }
    ...
    {
        $title = config('notifications.pending_confirm.title');
        $message = config('notifications.pending_confirm.message');

        $title = str_replace('{type}', 'tài xế', $title);
        $message = str_replace('{order_code}', $order->code, $message);
        $message = str_replace('{type}', 'tài xế', $message);

        $this->sendFirebaseNotificationToDriver($order->driver, $title, $message, MessageType::UNCLASSIFIED);
    }
    ...
    public function pendingUserConfirmation($order)
    {
        ...
        $title = str_replace('{type}', 'khách hàng', $title);
        $message = str_replace('{order_code}', $order->code, $message);
        $message = str_replace('{type}', 'khách hàng', $message);


        $this->sendFirebaseNotificationToUser($order->user, $title, $message, MessageType::UNCLASSIFIED);
    }

    public function driverConfirmedOrder($order)
    {
        $title = config('notifications.order_confirmed.title');
        $message = config('notifications.order_confirmed.message');
    ...
    {
        $title = config('notifications.order_declined.title');
        $message = config('notifications.order_declined.message');
    ...
    {
        $title_user = config('notifications.inTransitOrder_user.title');
        $message_user = config('notifications.inTransitOrder_user.message');

        $title_driver = config('notifications.inTransitOrder_driver.title');
        $message_driver = config('notifications.inTransitOrder_driver.message');

        $title_user = str_replace('{order_code}', $order->code, $title_user);
        $message_user = str_replace('{order_code}', $order->code, $message_user);

        $title_driver = str_replace('{order_code}', $order->code, $title_driver);
        $message_driver = str_replace('{order_code}', $order->code, $message_driver);

        $this->sendFirebaseNotificationToUser($order->user, $title_user, $message_user, MessageType::UNCLASSIFIED);
        $this->sendFirebaseNotificationToDriver($order->driver, $title_driver, $message_driver, MessageType::UNCLASSIFIED);
    }
    ...
    public function completedOrder($order)
    {
        ...
        $title_driver = config('notifications.completedOrder_driver.title');
        $message_driver = config('notifications.completedOrder_driver.message');
    ...
    {
        $title = config('notifications.cancelOrder.title');
        $message = config('notifications.cancelOrder.message');
    ...
    {
        $title = config('notifications.cancelOrder_type.title');
        $message = config('notifications.cancelOrder_type.message');
    ...
    {
        $title = config('notifications.onwayPickUpOrder.title');
        $message = config('notifications.onwayPickUpOrder.message');
    ...
    {
        $title = config('notifications.returnedOrder.title');
        $message = config('notifications.returnedOrder.message');
